<template>
  <ul class="navs-list">
    <li :class="[`space-lock-${level}`]" v-for="nav in data" :key="nav.name">
      <router-link v-if="nav.to" :to="nav.to" class="nav-item">
        {{ nav.label }}
      </router-link>
      <span v-else class="nav-item is-parent">{{ nav.label }}</span>
      <nav-list v-if="nav.children" :data="nav.children" :level="level + 1" />
    </li>
  </ul>
</template>
