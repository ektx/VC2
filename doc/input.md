...
通过设置 `autosize` 属性可以使得文本域的高度能够根据文本内容自动进行调整，并且 `autosize` 还可以设定为一个对象，指定 minRows最小行数和 maxRows最大行数。

- `minRows` 最小行数,默认最少2行，最大行数不限
...
<template>
  ...
  <vc-input type="textarea" :autosize="{ minRows: 3, maxRows: 6}" v-model="textarea2" />
</template>
